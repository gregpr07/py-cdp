...
class Launcher:
    ...
    def launch(self, timeout: float = 30.0) -> str:
        ...
        with self._lock:
            ...
            try:
                ...
                self.logger.info(
                    "Browser process started with PID %d", self._process.pid
                )
    ...
    def cleanup(self) -> None:
        ...
        if (
            ...
            and "bu" in self.config.user_data_dir
            and "session-" in self.config.user_data_dir
        ):
            ...
            if user_data_path.exists():
                ...
                try:
                    ...
                    self.logger.info(
                        "Cleaned up user data directory: %s", user_data_path
                    )
                except Exception as e:
    ...
    def _build_command(self, browser_path: Path) -> List[str]:
        ...
        if self.config.user_data_dir:
            flag_manager.set(
                Flag.USER_DATA_DIR, str(Path(self.config.user_data_dir).absolute())
            )
        ...
        if self.config.window_size:
            flag_manager.set(
                Flag.WINDOW_SIZE,
                f"{self.config.window_size[0]},{self.config.window_size[1]}",
            )

        if self.config.window_position:
            flag_manager.set(
                Flag.WINDOW_POSITION,
